...
def run(args):
    ...
    model = getattr(importlib.import_module(args.cam_network), 'Net_CAM_Feature')(n_classes=args.num_classes)
    param_groups = model.trainable_parameters()
    model.load_state_dict(torch.load(args.cam_weights_name), strict=True)
    model = torch.nn.DataParallel(model).cuda()

    recam_predictor = net.resnet50_cam.Class_Predictor(args.num_classes, 2048)
    ...
    train_dataset = mscoco.dataloader.COCOClassificationDataset(
        ...
        resize_long=(340, 340), hor_flip=True, crop_size=210, crop_method="random") #resize_long=(240, 340), crop_size=512
    ...
    val_dataset = mscoco.dataloader.COCOClassificationDataset(
        image_dir = osp.join(args.mscoco_root,'val2017/'),
        anno_path= osp.join(args.mscoco_root,'annotations/instances_val2017.json'),
        labels_path='./mscoco/val_labels.npy',crop_size=210) # crop_size=512
